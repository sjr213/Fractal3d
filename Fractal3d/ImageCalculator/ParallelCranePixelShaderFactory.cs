...
{
    public class ParallelCranePixelShaderFactory : IDisposable
    {
 ...
 //           var transViewPos = TransformationCalculator.Transform(transformMatrix, viewPos);
            ...
            for (var x = raw.FromWidth; x <= raw.ToWidth; ++x)
            {
                for (var y = 0; y < size.Height; ++y)
                {
                    ...
                    var distance = IntersectQJulia(ref startPt, direction, fractalParams, _nextCycle) * fractalParams.DistanceScale;
